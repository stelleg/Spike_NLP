#!/usr/bin/env python
"""
Model runner for FCN model using ESM embeddings (single target).
"""
import os
import tqdm
import torch
...
if __name__=='__main__':
    ...
    run_dir = os.path.join(results_dir, f"fcn-ESM_CLS-DMS_OLD-{result_tag}-{date_hour_minute}")
    ...
    save_as = f"fcn-ESM_CLS-DMS_OLD_{result_tag}-train_{len(train_dataset)}_test_{len(test_dataset)}"
