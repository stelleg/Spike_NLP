...
class BERT_BE(nn.Module):
    def __init__(self, bert, vocab_size):
        ...
        self.binding_head = nn.Linear(self.bert.hidden, 1)
        self.expression_head = nn.Linear(self.bert.hidden, 1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        binding_pred = self.binding_head(x).squeeze(1) # [batch_size, sequence_length, 1]
        expression_pred = self.expression_head(x).squeeze(1) # [batch_size, sequence_length, 1]
...
def epoch_iteration(model, tokenizer, mlm_loss_fn, regression_loss_fn, optimizer, data_loader, epoch, max_batch, device, mode):
    ...
    for batch, batch_data in data_iter:
        ...
        if mode == 'train':
            ...
            print(f"bind: {binding_preds.shape}, exp: {expression_preds.shape}")
