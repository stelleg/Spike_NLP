...
class FCN(nn.Module):
    ...
    def forward(self, x):
        fcn_out = self.fcn(x)
        prediction = self.out(fcn_out).squeeze(1)  # [batch_size, 1]
...
if __name__=='__main__':
    ...
    train_dataset = DMSEmbeddedDataset(os.path.join(data_dir, "pt/mutation_combined_DMS_OLD_train_ESM-CLS-embedded.pt"), result_tag)
    ...
    test_dataset = DMSEmbeddedDataset(os.path.join(data_dir, "pt/mutation_combined_DMS_OLD_test_ESM-CLS-embedded.pt"), result_tag)
    ...
    save_as = f"fcn-ESM_CLS_preembedded-DMS_OLD_{result_tag}-train_{len(train_dataset)}_test_{len(test_dataset)}"
