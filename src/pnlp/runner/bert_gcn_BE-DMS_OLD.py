#!/usr/bin/env python
"""
Model runner for BERT-GCN model (multi target for both binding and expression).
...
class BERT_GCN(nn.Module):
    ...
    def forward(self, x, binding_targets, expression_targets):
        with torch.set_grad_enabled(self.training):  # Enable gradients, managed by model.eval() or model.train() in epoch_iteration
            ...
            # Graph Construction
            graphs = []
            for embedding, b_target, e_target in zip(bert_embedding, binding_targets, expression_targets):
            ...
            error_2_binding, error_2_expression = self.gcn(batch_graph.x, batch_graph.edge_index, batch_graph.batch)

        return error_1, error_2_binding, error_2_expression, batch_graph.y
...
def epoch_iteration(model, tokenizer, mlm_loss_fn, regression_loss_fn, optimizer, data_loader, epoch, max_batch, device, mode):
    ...
    for batch, batch_data in data_iter:
        ...
        if mode == 'train':
            optimizer.zero_grad()
            mlm_preds, binding_preds, expression_preds, y = model(masked_tokenized_seqs, binding_targets, expression_targets)
            batch_mlm_loss = mlm_loss_fn(mlm_preds.transpose(1, 2), unmasked_tokenized_seqs)
            batch_binding_loss = regression_loss_fn(binding_preds, y[:, 0])
            batch_expression_loss = regression_loss_fn(expression_preds, y[:, 1])
...
if __name__=='__main__':
    ...
    save_as = f"bert_gcn_BE-DMS_OLD-train_{len(train_dataset)}_test_{len(test_dataset)}"
