...
if __name__=='__main__':
    ...
    results_dir = os.path.join(os.path.dirname(__file__), f'../../../results/run_results/esm_blstm-bert_mlm_esm_init')

    # Create run directory for results
    now = datetime.datetime.now()
    date_hour_minute = now.strftime("%Y-%m-%d_%H-%M")
    run_dir = os.path.join(results_dir, f"adam.lr{lr}.esm_blstm-bert_mlm_esm_init-DMS_OLD-{result_tag}-{date_hour_minute}")
    ...
    save_as = f"esm_blstm-bert_mlm_esm_init-DMS_OLD_{result_tag}-train_{len(train_dataset)}_test_{len(test_dataset)}"
