#!/usr/bin/env python
"""
Model runner for FCN model using ESM embeddings (multi target for both binding and expression).
"""
import os
import re
import sys
...
def run_model(model, tokenizer, embedder, train_data_loader, test_data_loader, n_epochs: int, lr:float, max_batch: Union[int, None], device: str, run_dir: str, save_as: str, saved_model_pth:str=None, from_checkpoint:bool=False):
    ...
    for epoch in range(starting_epoch, n_epochs + 1):
        train_binding_mse, train_binding_rmse, train_expression_mse, train_expression_rmse, train_mse, train_rmse = epoch_iteration(model, tokenizer, embedder, loss_fn, optimizer, train_data_loader, epoch, max_batch, device, mode='train')
        test_binding_mse, test_binding_rmse, test_expression_mse, test_expression_rmse, test_mse, test_rmse = epoch_iteration(model, tokenizer, embedder, loss_fn, optimizer, test_data_loader, epoch, max_batch, device, mode='test')
...
if __name__=='__main__':
    ...
    results_dir = os.path.join(os.path.dirname(__file__), f'../../../results/run_results/fcn-ESM_CLS')

    # Create run directory for results
    now = datetime.datetime.now()
    date_hour_minute = now.strftime("%Y-%m-%d_%H-%M")
    run_dir = os.path.join(results_dir, f"fcn-ESM_CLS-DMS_OLD_BE-{date_hour_minute}")
    ...
    save_as = f"fcn-ESM_CLS-DMS_OLD_BE-train_{len(train_dataset)}_test_{len(test_dataset)}"
