...
def epoch_iteration(model, tokenizer, embedder, loss_fn, optimizer, data_loader, epoch, max_batch, device, mode):
    ...
    for batch, batch_data in data_iter:
        ...
        # Pre-embed sequences
        with torch.no_grad():
            embeddings, _ = embedder(tokenized_seqs)
...
if __name__=='__main__':
    ...
    results_dir = os.path.join(os.path.dirname(__file__), f'../../../results/run_results/blstm-NLP')

    # Create run directory for results
    now = datetime.datetime.now()
    date_hour_minute = now.strftime("%Y-%m-%d_%H-%M")
    run_dir = os.path.join(results_dir, f"blstm-NLP-DMS_OLD_BE-{date_hour_minute}")
    ...
    save_as = f"blstm-NLP-DMS_OLD_BE-train_{len(train_dataset)}_test_{len(test_dataset)}"
